segment <- function(x, weights=NULL, alpha=0.01, nperm=10000, p.method= 
                    ...
                    verbose=1) {
  if (class(x) != "CNA") stop("First arg must be a CNA object")
  call <- match.call()
...
# rudimentary error checking for weights
  if (weighted) {
    if (length(weights) != nrow(x@genomdat)) stop("length of weights should be the same as the number of probes")
...
#    maploci <- x$maploc[ina]
  ...
  if (is.ordered(x$chrom)) allsegs$chrom <- levels(x$chrom)[allsegs$chrom]
  allsegs$ID <- sampleid[allsegs$ID]
  allsegs$seg.mean <- round(allsegs$seg.mean, 4)
  allsegs <- as.data.frame(allsegs)
  segRows <- as.data.frame(segRows)
  allsegs$ID <- as.character(allsegs$ID)
  segres <- new("DNAcopy", x, call=call, output=allsegs, segRows = segRows)
  segres
}
