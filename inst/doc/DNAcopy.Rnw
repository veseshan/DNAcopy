...
when there is a strong evidence for its presence \citep{venkat06}.  We no
...
within teh function {\tt segment} or can be done externally using the {\tt
  getbdry} and passed on to {\tt segment}. 
